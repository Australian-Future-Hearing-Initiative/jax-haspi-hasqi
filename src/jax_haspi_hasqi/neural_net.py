"""HASPI v2 neural-network ensemble, ported from clarity.evaluator.haspi.ip.

The weights are the published HASPI v2 constants, transcribed from the
reference. Ten networks: 10 inputs plus bias, 4 hidden units, 1 output,
logistic activation throughout.
"""
...
_OUTPUT_WEIGHTS = np.array(
  [
    [-0.1316, -2.5182, 1.6401, -3.2093, 1.7924],
    [-0.1653, 1.7375, 1.5526, -3.2349, -2.2877],
    [0.1847, -3.1987, -2.4941, 2.7106, -1.8048],
    [0.3962, -3.2952, 3.0003, -2.2602, -2.3269],
    [-0.0646, 1.3288, -3.4087, -2.0046, 1.8565],
    [1.3676, -3.4129, 1.6895, -1.8913, -1.5595],
    [0.8124, 2.7171, -3.0867, -2.3310, -2.3657],
    [-0.2743, 1.4949, 0.7896, -4.0589, 1.1257],
    [0.1307, 2.2788, -2.3633, -1.5073, -2.9985],
    [0.1024, -0.9517, 2.2123, -2.4008, -3.1655],
  ]
)

# Divides the ensemble mean so the maximum network output is exactly 1.
_NORMALIZATION = 0.9508


def nn_feed_forward_ensemble(features):
  """Mean of the ten networks' outputs for one feature vector.

  Args:
    features: The ten modulation-band correlations.

  Returns:
    The ensemble mean, before normalisation.
  """
  augmented = jnp.concatenate([jnp.ones(1), jnp.asarray(features)])
  hidden = jax.nn.sigmoid(jnp.einsum("i,nij->nj", augmented, _HIDDEN_WEIGHTS))
  hidden = jnp.concatenate([jnp.ones((hidden.shape[0], 1)), hidden], axis=1)
  output = jax.nn.sigmoid(jnp.sum(_OUTPUT_WEIGHTS * hidden, axis=1))
  return jnp.mean(output)


def intelligibility(features):
  """Normalised HASPI intelligibility estimate."""
  return nn_feed_forward_ensemble(features) / _NORMALIZATION
